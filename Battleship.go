...
func (game *Battleship) PlaceShips() {
	...
	for i := 0; i < 2; i++ {
		...
		if err != nil || y < 0 || y > 7 || x < 'A' || x > 'H' || (isHorizontal && x == 'A') || (!isHorizontal && y == 0) {
			fmt.Println("<!> Invalid placement. Ships are place from the center out")
			i--
		} else {
			if isHorizontal {
				for i := int(x) - int('A') - 1; i < game.currentPlayer.GetShip().GetHitpoints(); i++ {
					game.currentPlayer.PlaceShip(MakeCoord(rune(i)+'A', y))
				}
			} else {
				for i := y - 1; i < game.currentPlayer.GetShip().GetHitpoints(); i++ {
					game.currentPlayer.PlaceShip(MakeCoord(x, i))
				}
			}
			game.EndTurn()
		}
